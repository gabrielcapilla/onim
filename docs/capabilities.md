...
| Completion                               | native / partial       | Locals, imported module members, project surfaces, standard-library surfaces, implicit `File` receivers from stdlib metadata, known object fields, inferred named-tuple sequence elements, unary explicit generic-object fields, and exact same-file or directly imported-project UFCS procedures, including explicit unary primitive generic receivers. General expression-type completion remains unsupported. |
| Type propagation                         | partial                | Primitive literals including width-suffixed `int8`/`uint8`/`float32` forms, canonical primitive annotations, explicit named/primitive `array[...]` and one-level primitive `seq[...]` annotations, homogeneous inferred sequences of named tuple literals, one-level `ref` wrappers, and unambiguous direct routine returns including exact primitive `seq[...]` results. |
