...
""",
    ...
    check privateReferences.supported
    check privateReferences.matches.len == 2
    var privateDeclaration = false
    var privateUse = false
    for match in privateReferences.matches:
      check match.fileId.value == providerId.value
      let token = privateSnapshot.index.parsed.tokens[int(match.tokenIndex)]
      if token.startOffset == privateSnapshot.text.find("hidden"):
        privateDeclaration = true
      if token.startOffset == privateSnapshot.text.rfind("hidden"):
        privateUse = true
