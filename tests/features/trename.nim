...
"""
    ...
    let privateInfo = resolveRename(
      workspace, providerSnapshot, providerSource.find("hidden") + 1, "visible"
    )
    check privateInfo.state == renameAvailable
    check privateInfo.matches.len == 2
    check matchOffsets(workspace, privateInfo, providerPath) ==
      @[providerSource.find("hidden"), providerSource.rfind("hidden")]
    check resolveRename(
      workspace, providerSnapshot, providerSource.find("hidden") + 1, "display"
    ).state == renameUnavailable
