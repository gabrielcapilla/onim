...
suite "native hover":
  test "uses marker positions for project documentation hover":
    let fixture =
      parseFixture("## Greets the caller.\nproc greet*() = discard\ngre<|>et()\n")
    let info = hoverAt(fixture.files["main.nim"], fixture.cursors[0].byteOffset)
    check info.state == hoverAvailable
    check info.name == "greet"
    check info.documentation.contains("Greets the caller")
  ...
  test "hovers imported stdlib modules":
    let source = "import std/strformat\nfmt(\"hi\")\n"
    let info = hoverAt(source, source.find("strformat") + 1)
    check info.state == hoverAvailable
...
"""
  ...
  test "resolves declarations inside formatted string interpolation":
    let source = """import std/strformat
let number = 190_000
...
"""
    let info = hoverAt(source, source.find("{number}") + 2)
    check info.state == hoverAvailable
    check info.name == "number"
    check info.signature == "let number: uint32"
    check info.declarationLine == 2
    check info.declarationText == "let number = 190_000"
  ...
  test "hovers project module documentation":
    let root = getTempDir() / ("onim-hover-module-" & $getCurrentProcessId())
    if dirExists(root):
      removeDir(root)
    createDir(root)
    let providerPath = root / "provider.nim"
    let consumerPath = root / "consumer.nim"
    let provider = "## Supplies project values.\nproc answer*(): int = 42\n"
    ...
    let info =
      resolveHover(workspace, snapshot, consumer.find("provider") + 1, stdlibMap())
    check info.state == hoverAvailable
    check info.kind == "module"
    check info.module == "provider"
    check info.documentation == "Supplies project values."
...
"""
    ...
    check hoverFor(source, "count").signature == "let count: uint8"
    check hoverFor(source, "ratio").signature == "let ratio: float"
    check hoverFor(source, "mask").signature == "let mask: uint8"
    check hoverFor(source, "numbers").signature == "let numbers: seq[uint8]"
...
"""
    ...
    check hoverFor(source, "unary").signature == "let unary: int8"
    for name in ["prefixed", "missing", "compound", "mixed", "empty"]:
...
"""
    ...
    check hoverAt(source, outerOffset).signature == "let item: uint8"
