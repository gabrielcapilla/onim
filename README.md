...
On-disk source indexes are cached under `ONIM_CACHE_DIR` when set, then `XDG_CACHE_HOME/onim`, or `~/.cache/onim`. A project manifest records the canonical module inventory and file stamps; per-module records are keyed by canonical project/module paths and exact source fingerprints. The records store structured tokens and import metadata in versioned, checksummed files, so a restart can reuse an unchanged module without lexing it again. Cache data is acceleration only: an identity, version, checksum, bounds, or source mismatch falls back to a fresh in-memory index.
